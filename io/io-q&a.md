...
### 37. De ce subsistemul de networking nu folosește buffer cache-ul?
   ...
   Subsistemul de networking poate sa se foloseasca de buffer cache, dar nu o face din motive de performanta.
   
   Buffer cache-ul este destinat gestionarii si stocarii datelor care (1) se afla in procesare sau (2) sunt accesate frecvent, in memoria locala a unui sistem. Subsistemul de retea se ocupa cu transmisia de date intre sisteme si poate gestiona un volum mare de date.
   Este necesara o procesare mai rapida decat cea oferita de cache, deci utilizarea acestuia nu ar avea sens.
